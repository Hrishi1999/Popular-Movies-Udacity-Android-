...
public class MoviesContentProvider extends ContentProvider {
    ...
    public Cursor query(@NonNull Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        switch (match) {
            case MOVIES:
                cursor = db.query(MovieDBContract.Movie.TABLE_NAME,
                        projection,
                        selection,
                        selectionArgs,
                        null,
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            case MOVIES:
                long id = db.insert(MovieDBContract.Movie.TABLE_NAME, null, values);
                if (id > 0) {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case MOVIES:
                rowDeleted = db.delete(MovieDBContract.Movie.TABLE_NAME, selection, selectionArgs);
                break;
            default:
                db.close();
